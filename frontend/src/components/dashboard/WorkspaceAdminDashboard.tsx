...
const WorkspaceAdminDashboard: React.FC = () => {
  ...
  // 3. Quản lý thành viên chi tiết của từng Dự án (mở rộng theo dòng)
  const [expandedProjectId, setExpandedProjectId] = useState<number | null>(null);
  const [selectedAddMemberId, setSelectedAddMemberId] = useState<number>(0);

  // 4. Tạo workspace mới (mock notice modal)
  ...
  const handleAddProjectMember = async (projectId: number) => {
    ...
    try {
      await workspaceService.addProjectMember(projectId, selectedAddMemberId);
      setSuccessMsg("Đã thêm thành viên vào dự án thành công.");
      setSelectedAddMemberId(0);
      // Nạp lại dự án
      const projectsData = await workspaceService.getProjects();
      setProjects(projectsData);
    } catch (err: any) {
  ...
  const handleRemoveProjectMember = async (projectId: number, memberUserId: number) => {
    if (!window.confirm("Bạn có chắc chắn muốn gỡ thành viên này khỏi dự án?")) return;
    try {
      await workspaceService.removeProjectMember(projectId, memberUserId);
